...
namespace SAM.Picker
{
    internal partial class GamePicker : Form
    {
        ...
        private async void OnUnlockAll(object sender, EventArgs e)
        {
            ...
            if (games.Count == 0)
            {
                MessageBox.Show(
                    this,
                    "No games in the list to process.",
                    "Information",
                    MessageBoxButtons.OK,
            ...
            if (File.Exists(samGameExe) == false)
            {
                MessageBox.Show(
                    this,
                    $"SAM.Game.exe was not found.\n\nSearched in:\n{exeDir}\n\n" +
                    "Rebuild the full solution so all executables are placed in the same output directory.",
                    "Error",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
                return;
        ...
        private async Task<(int succeeded, int failed, bool criticalFailure)> RunUnlockAllIterationAsync(List<GameInfo> games, int currentLoop, int totalLoops, bool infinite)
        {
            ...
            foreach (var game in games)
            {
                ...
                {
                    this._PickerStatusLabel.Text =
                        $"Failed to start SAM.Game.exe for {game.Name}: {ex.Message}";
                    failed++;
                    continue;
                }
